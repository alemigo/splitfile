...
#-----------------------------
#
"""Python file-like object that facilitates reading and writing of binary data
   split across multiple volumes (files) of a specified size. Includes support for
...
class SplitFile(object):
    ...
    file_name = None        # Filename of first volume, or iterable of volume filenames to use
    ...
    aes_key = None          # Encryption key for AES, or None for unencrypted operation
    ...
    def __init__(self, filename, mode, volume_size=0, compression=False, lzma_preset=6, aes_key=None):
    ...
    def _next_file(self):
        """Close prior volume if open, and open next volume (if possible in read mode)"""
        if self.file:
            self.file.close()
            self.file = None
            self.file_size = 0
        ...
        # Open the file or return false if in read mode and next file doesn't exist
    ...
    def write(self, data):
        ...
        if self.mode != 'wb': raise io.UnsupportedOperation('Cannot write in read mode')
        ...
        if self.volume_size > 0:
            ...
            while len(data) > 0:
                ...
                while write_size == 0:
                    ...
                    write_size = min(len(data), self.volume_size - self.file_size)
                    if write_size > 0:
                        break
                    else:
                        self._next_file()
...
def open(filename, mode, volume_size=0, compression=False, lzma_preset=6, aes_key=None):
    """alternative constructor"""
    return SplitFile(filename, mode, volume_size, compression, lzma_preset, aes_key)
